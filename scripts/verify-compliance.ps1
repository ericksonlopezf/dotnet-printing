...
<#
.SYNOPSIS
    Automated zero-tolerance repository compliance & governance verifier for EricksonLopez.Printing.
.DESCRIPTION
    Validates:
...
$allCsFiles = Get-ChildItem -Path $RootDirectory -Recurse -Filter "*.cs" | Where-Object { $_.FullName -notmatch "[\\/](obj|bin)[\\/]" }
...
foreach ($extra in $extraFiles) {
    if (Test-Path $extra) {
        $firstLine = (Get-Content $extra -TotalCount 1 -Encoding UTF8)
        if ($firstLine -notmatch "Copyright .* Erickson Lopez.* MIT License") {
...
if ($propsContent -notmatch "<PackageProjectUrl>https://ericksonlopez\.dev/printing</PackageProjectUrl>") {
    Write-Host "  ❌ PackageProjectUrl must be set to https://ericksonlopez.dev/printing." -ForegroundColor Red
    $violations++
}
Write-Host "  ✅ Directory.Build.props invariants and icon.png verified." -ForegroundColor Green

# 7. NoWarn Governance Audit (Zero Exceptions)
...
if (-not $strykerConfigFiles -or $strykerConfigFiles.Count -eq 0) {
    ...
    $isSolutionConfig = $false
    if ($strykerProfiles.Count -eq 1 -and $strykerProfiles[0].Name -eq "stryker-config.json") {
        $rawConfig = Get-Content $strykerProfiles[0].FullName -Raw | ConvertFrom-Json
        $cfgObj = if ($rawConfig.PSObject.Properties['stryker-config']) { $rawConfig.'stryker-config' } else { $rawConfig }
        if ($cfgObj.PSObject.Properties['solution']) {
            $isSolutionConfig = $true
            # Verify all projects are covered in mutate
            $mutateRaw = ($cfgObj.mutate | Out-String)
            foreach ($proj in $srcProjects) {
                if ($mutateRaw -notmatch [regex]::Escape($proj.BaseName)) {
                    Write-Host "  ❌ Project '$($proj.Name)' is missing from mutate array in stryker-config.json." -ForegroundColor Red
                    $violations++
                    $strykerErrors++
                }
            }
    ...
    if (-not (Test-Path $mutationWfPath)) {
        ...
        foreach ($sf in $strykerConfigFiles) {
            ...
            if ($wfContent -notmatch [regex]::Escape($sf.Name) -and $wfContent -notmatch "(?i)name:\s*$pkgIdent" -and $wfContent -notmatch "(?i)working-dir:.*$pkgIdent") {
                ...
                if (Get-Variable -Name "violations" -Scope 0 -ErrorAction SilentlyContinue) { $violations++ }
                if (Get-Variable -Name "Violations" -Scope 0 -ErrorAction SilentlyContinue) { if ($Violations -is [System.Collections.IList]) { $Violations.Add("Stryker configuration '$($sf.Name)' is missing from matrix") } }
                if (Get-Variable -Name "FailedChecks" -Scope 0 -ErrorAction SilentlyContinue) { $FailedChecks++ }
                $strykerErrors++
            }
        }
...
if ($aotApplicableProjects.Count -gt 0) {
    ...
    if ($aotTestProjects.Count -eq 0) {
        Write-Host "  [INFO] Native AOT trimming verified via <IsAotCompatible>true</IsAotCompatible> and <EnableTrimAnalyzer>true</EnableTrimAnalyzer> in Directory.Build.props." -ForegroundColor Gray
    } else {
    ...
    # 3. Check for CI workflow .github/workflows/aot-smoke-test.yml (if configured)
    $aotWorkflowPath = Join-Path $targetRoot ".github/workflows/aot-smoke-test.yml"
    if (Test-Path $aotWorkflowPath) {
        ...
        Write-Host "  [INFO] Dedicated AOT workflow not present; Native AOT compatibility validated via compiler Roslyn analyzers in ci.yml." -ForegroundColor Gray
    }
} else {
