...
public sealed class PrinterHubTests
{
    ...
    [Fact]
    public async Task RegisterPrinter_ConfiguresAwaitFalse()
    {
        ...
        groups.AddToGroupAsync(Arg.Any<string>(), Arg.Any<string>(), Arg.Any<CancellationToken>())
            .Returns(_ => Task.Run(async () => await Task.Delay(10).ConfigureAwait(false)));
        ...
        await Task.Run(async () =>
        {
            ...
            try
            {
                await hub.RegisterPrinter("POS-01").ConfigureAwait(false);
            }
            finally
            {
    ...
    [Fact]
    public async Task UnregisterPrinter_ConfiguresAwaitFalse()
    {
        var syncContext = new DisallowingSynchronizationContext();
        var groups = Substitute.For<IGroupManager>();
        ...
        await Task.Run(async () =>
        {
            ...
            try
            {
                await hub.UnregisterPrinter("POS-01").ConfigureAwait(false);
            }
            finally
            {
                SynchronizationContext.SetSynchronizationContext(null);
