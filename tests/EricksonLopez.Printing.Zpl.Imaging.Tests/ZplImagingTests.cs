...
public sealed class ZplImagingTests
{
    ...
    [Fact]
    public void BuildGraphicFieldCommand_InvalidDimensions_ThrowsArgumentOutOfRangeException()
    {
        var actWidth = () => ZplGraphicFieldConverter.BuildGraphicFieldCommand(0, 0, 0, 10, [0xFF]);
        actWidth.Should().Throw<ArgumentOutOfRangeException>()
            .WithMessage("*Width must be positive.*")
            .WithParameterName("width");

        var actHeight = () => ZplGraphicFieldConverter.BuildGraphicFieldCommand(0, 0, 10, 0, [0xFF]);
