...
public static class MonochromeBitmapConverter
{
        ...
        byte scale = 0)
    {
        ...
        if (packedRaster.Length < (long)widthInBytes * height)
        {
            throw new ArgumentException("Packed raster data too small.", nameof(packedRaster));
        }

        var xL = (byte)(widthInBytes & 0xFF);
        // Stryker disable once Bitwise : widthInBytes is positive, so >> and >>> are bitwise equivalent
        var xH = (byte)((widthInBytes >> 8) & 0xFF);
        var yL = (byte)(height & 0xFF);
        // Stryker disable once Bitwise : height is positive, so >> and >>> are bitwise equivalent
