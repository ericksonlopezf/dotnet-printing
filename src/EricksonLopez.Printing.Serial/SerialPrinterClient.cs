...
public sealed class SerialPrinterClient : IPrinterClient, IDisposable
{
    ...
    /// <exception cref="ArgumentNullException"><paramref name="options"/> is <see langword="null"/></exception>
    public SerialPrinterClient(SerialPrinterClientOptions options, ILogger<SerialPrinterClient>? logger)
    {
        _options = options ?? throw new ArgumentNullException(nameof(options));
        _logger = logger;
    }

    internal Func<SerialPrinterClientOptions, Stream>? StreamFactory { get; set; }
    ...
    public async Task<Result<bool>> PrintAsync(IPrintDocument document, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            {
                Stream stream;
                IDisposable? port = null;

                if (StreamFactory is not null)
                {
                    stream = StreamFactory(_options);
                    port = stream;
                }
                else
                {
                    // Stryker disable all : Hardware-bound SerialPort initialization cannot be exercised without physical hardware
