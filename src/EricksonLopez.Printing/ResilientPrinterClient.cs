...
public sealed class ResilientPrinterClient : IPrinterClient
{
    ...
    public async Task<Result<bool>> PrintAsync(IPrintDocument document, CancellationToken cancellationToken = default)
    {
        ...
        while (true)
        {
            ...
            if (ShouldSkipNonIdempotentRetry(document, result.Error))
            {
                _logger?.LogWarning(
                    PrintingEventIds.RetryExhausted,
    ...
    private bool ShouldSkipNonIdempotentRetry(IPrintDocument document, Error error)
    {
        ...
        return error.Code == PrintingErrorCodes.TransmissionError || error.Code == PrintingErrorCodes.Timeout;
    }
    ...
    private TimeSpan CalculateNextDelay(TimeSpan currentDelay)
    {
        ...
        return TimeSpan.FromMilliseconds(Math.Min(nextDelayMs, _options.MaxDelay.TotalMilliseconds));
    }
}
