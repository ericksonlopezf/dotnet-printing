...
public sealed class PooledTcpPrinterClient : IPrinterClient, IAsyncDisposable, IDisposable
{
    ...
    internal void InvalidateStreamForTesting() => _stream = null;
    ...
    /// <exception cref="ArgumentNullException"><paramref name="options"/> is <see langword="null"/></exception>
    public PooledTcpPrinterClient(TcpPrinterClientOptions options, ILogger<PooledTcpPrinterClient>? logger)
    {
    ...
    {
        if (_client is { Connected: true } && _stream is not null && !IsSocketDisconnected(_client.Client))
        {
            return;
        }
    ...
    internal static bool IsSocketDisconnected(Socket socket) =>
